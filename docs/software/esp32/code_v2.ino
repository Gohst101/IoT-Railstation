...
void wifi_connection() {
  ...
  if (use_status_led == 1) digitalWrite(wlan_pin, LOW); // Immer aus am Anfang
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (use_status_led == 1 && millis() - lastToggle >= status_led_blink_time) {
      ledState = !ledState;
...
void mqtt_connection() {
  ...
  if (use_status_led == 1) digitalWrite(mqtt_pin, LOW); // Immer am anfang aus
